...
\descrbox{What to submit}{
  Each group has to send, via email to \bash{drp@isep.ipp.pt}, the two Python files that contain the functions that are required to be implemented. 
}
...
We will now look into the VC\_i function, available in file \bash{VCs.py}, and that implements the improved verification condition generation algorithm introduced in the classes. First, we recall the algorithmic specification of the function:
\begin{align*}
...
To be able to use this driver, you must install the corresponding Python module. The module is named Z3Python and can be installed via the Python package manager \bash{pip}.
