...
Initially the \compn{controller} is idle, where he must remain for at least 1000ms to perform some bootstrap self-tests. It can then receive a \chn{start} command to become ready to actuate.
...
The \compn{controller} then waits between 4000ms and 5000ms -- if a report is received from the \compn{motor} signalling that it reached the movement \chn{limit}, the \compn{controller} will become idle and will notify the \compn{dashboard} that the \compn{motor} stopped. Otherwise, after 5000s the controller raises an \chn{error} (more info on this below) and goes to a fallback state.
...
Every 100ms each of the two \compn{controllers} should check if the other controller is in the same state, using shared variables. After 3 failed attempts this controller should raise an error and go to the fallback state.

To be able to test this \compn{controllers}, the \compn{dashboard} can also send a \chn{fail} command to any of the \compn{controllers}, rendering it useless.
...
\begin{myExercise} \label{ex:req}
  Write a set of desirable requirements following the EARS approach based on the description of this system.
\end{myExercise}
